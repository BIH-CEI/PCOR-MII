...
2. **Rendering** – a client (ePRO app, study portal) renders the items based on `type`, `text`, and answer options.
3. **Pre-population** *(optional)* – existing data is pre-filled (e.g. via [Structured Data Capture, SDC](https://hl7.org/fhir/uv/sdc/)).
4. **Capture** – the completed answers are stored as a `QuestionnaireResponse` (`status = completed`).
5. **Evaluation / extraction** *(optional)* – relevant answers are transferred into other FHIR resources (e.g. `Observation`) via data extraction.
